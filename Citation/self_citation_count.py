...
data = pd.read_csv(r"D:\\dissertation\\data\\profile_data5.csv",encoding='ISO-8859-1')
author_first_name=''
total=0
for i in range(len(data)):
    ...
    #data.at[i,'diff']=''
    print(total)
    if pd.isna(data.loc[i, 'citation'])==False:
        data.at[i, 'diff'] = int(data.at[i, 'citation'])-total
    else:
        data.at[i, 'diff'] =0
        ...
        #data.at[i,'%']=''
    if pd.isna(data.loc[i, 'citation'])==False:
        data.at[i, '%'] = (int(total)/int(data.at[i, 'citation']))*100
    else:
        data.at[i, '%'] =0
    #time.sleep(random.uniform(300, 500))
    data.loc[i, 'total_self_citation'] =''
    data.at[i, 'total_self_citation'] = total
data.to_csv("D:/dissertation/data/profile_data_self_citations.csv",mode='a', header=False,index=False)
